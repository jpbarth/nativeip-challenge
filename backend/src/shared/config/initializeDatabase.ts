...
import { citiesArray } from '../files/cities';
import { customersArray } from '../files/customers';
...
async function createCities(cities: ICity[]) {
  await Promise.all(
    cities.map(city =>
      City.findCreateFind({ where: { id: city.id }, defaults: { ...city } }),
    ),
  );
}
...
async function initializeDatabase() {
  ...
  await createCities(citiesArray);
  await createCustomers(customersArray);
}
